...
function about_phones_html($limit = false)
{
  ...
  if (isset($about['phones'])) {
    if (is_array($about['phones'])) {
      ...
      ?>
      <div class="contacts-block phones-block">
        <?php
              $count = 0;
              foreach ($about['phones'] as $phone) {
                if ($count === $total) break;
                ?>
          <div class="contact-row phone-row ico-left">
            <div class="ico ico-<?= $phone['operator']; ?>">
              <?= $svg->get_ico($phone['operator']); ?>
            </div>
  ...
  function about_emails_html($limit = false)
  {
    ...
    ?>
  <div class="contacts-block emails-block">
    <?php
      $count = 0;
